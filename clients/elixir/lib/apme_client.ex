defmodule ApmeClient do
  @moduledoc "HTTP client for the Apostille Me service API."
  ...
  def request(%__MODULE__{} = client, method, path, json \\ nil) do
    ...
    request = if is_nil(json), do: {url, headers}, else: {url, headers, ~c"application/json", json}

    case :httpc.request(method, request, [autoredirect: false], [body_format: :binary]) do
